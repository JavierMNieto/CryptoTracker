from django.http import HttpResponse
from django.template import loader
from neo4j.v1 import GraphDatabase
from . import constants
from btc.models import Node as btcNode
from usdt.models import Node as usdtNode
from django.shortcuts import render
import ccxt
import json
import time
import math

driver = GraphDatabase.driver(constants.neo4j['url'], auth=(constants.neo4j['user'], constants.neo4j['pass']))
models = {
	'btc': btcNode,
	'usdt': usdtNode
}

def numWithCommas(num):
	return ("{:,}".format(float(num)))

def search(request, coin=None):
	if coin is None:
		coin = 'usdt'
	cmc = ccxt.coinmarketcap()
	btc = cmc.fetch_ticker('BTC/USD')

	nodes = models[coin].objects.order_by('name')

	categories = [{
		'category': 'Home',
		'url': "/{}/search/0".format(coin),
		'addrs': []
	}]
	for node in nodes:
		categories[0]['addrs'].append({
			'addr': node.name,
			'url': "/{}/search/{}".format(coin, node.name)
		})
		if node.category == '':
			continue
		add = True
		for category in categories:
			if category['category'] == node.category:
				category['addrs'].append({
					'addr': node.name,
					'url': "/{}/search/{}".format(coin, node.name)
				})
				category['url'].append(node.name)
				add = False
				break
		if add:
			categories.append({
				'category': node.category,
				'url': ['.{}'.format(node.category), node.name],
				'addrs': [{
					'addr': node.name,
					'url': "/{}/search/{}".format(coin, node.name)
				}]
			})
	for category in categories:
		if category['category'] == 'Home':
			continue
		category['url'] = "/{}/search/{}".format(coin, json.dumps(category['url']))
	search = {'search': categories[0]['addrs'], 'categories': categories, 'coin': coin, 'homeUrl': '/{}/search/0'.format(coin), 'btc': btc}
	return render(request, 'tracker/index.html', search)

def usdt_home(request):
	return search(request, 'usdt')

def btc_home(request):
	return search(request, 'btc')

def home(request):
	cmc = ccxt.coinmarketcap()
	btc = cmc.fetch_ticker('BTC/USD')
	return render(request, 'tracker/index.html', {'search': [], 'coin': 'home', 'homeUrl': '#', 'btc': btc})

def searchHome(request):
	mainAddr = None
	cmc = ccxt.coinmarketcap()
	btcPrice = float(cmc.fetch_ticker('BTC/USD')['last'])
	
	with driver.session() as session:
		txs = session.run("MATCH (a)-[r]->(b) WHERE NOT r.isTotal "
						"RETURN a,b,r ORDER BY r.epoch DESC")
		addrs = session.run("MATCH (a)-[r]->(b) "
							"WITH DISTINCT a,b, count(r) AS sstcount "
							"MATCH p=(a)-[r]->(b) "
							"WHERE sstcount = 1 OR r.isTotal = True "
							"RETURN a, b, r")
		mainAddr = {
			"label": "All Addresses",
			"balance": 0
		}

	data = {
		'nodes': [],
		'edges': {
			'collapsed': [],
			'all': []
		}
	}

	for nodes in addrs:
		addrUrl = 'https://blockexplorer.com/address/'
		nodeType = 'btc'
		txUrl = 'https://blockexplorer.com/tx/'
		img = "data:image/png;base64,"

		aNode = nodes.get('a')
		if next(iter(aNode.labels)).lower() == 'usdt':
			addrUrl = 'https://omniexplorer.info/address/'
			nodeType = 'usdt'
			txUrl = 'https://omniexplorer.info/tx/'
			img = "https://s2.coinmarketcap.com/static/img/coins/200x200/825.png"

		aNode = {
			"id": aNode.id,
			"label": aNode['name'],
			"address": aNode['addr'],
			"balance": float(aNode['balance'] or 0),
			"balVal": float(aNode['balance'] or 0)*btcPrice if nodeType == 'btc' else float(aNode['balance'] or 0),
			"group": aNode['wallet'] or "",
			"lastUpdate": aNode['epoch'] or time.time(),
			"url": "/{}/search/{}".format(nodeType, aNode['name']) if aNode['minTx'] is not None else addrUrl + aNode['addr'],
			"webUrl": addrUrl + aNode['addr'],
			"walletName": aNode['walletName'] or "",
			"value": ((float(aNode['balance'] or 0)/100000000)*btcPrice) if nodeType == 'btc' else float(aNode['balance'] or 0)/100000000,
			"img": img,
			"title": ("<b>{}</b><br> "
						"Address: {}<br> "
						"Balance: {}<br> "
						"Wallet: {}<br> "
						"Last Updated: {}").format(nodeType.upper(), aNode['addr'], numWithCommas(float(aNode['balance'] or "0")), 
													"None" if aNode['walletName'] is None or aNode['walletName'] == '' else aNode['walletName'], aNode['lastUpdate'])
		}
		if aNode['label'] != aNode['address']:
			aNode['title'] = ("<b>{}</b><br> "
							"Name: {}<br> "
							"Address: {}<br> "
							"Balance: {}<br> "
							"Wallet: {}<br> "
							"Last Updated: {}").format(nodeType.upper(), aNode['label'], aNode['address'], numWithCommas(aNode['balance']), 
														"None" if aNode['walletName'] is None or aNode['walletName'] == '' else aNode['walletName'], 
														time.strftime("%Y-%m-%d, %H:%M:%S", time.localtime(aNode['lastUpdate'])))
		bNode = nodes.get('b')
		bNode = {
			"id": bNode.id,
			"label": bNode['name'],
			"address": bNode['addr'],
			"balance": float(bNode['balance'] or 0),
			"balVal": float(bNode['balance'] or 0)*btcPrice if nodeType == 'btc' else float(bNode['balance'] or 0),
			"group": bNode['wallet'] or "",
			"lastUpdate": bNode['epoch'] or time.time(),
			"url": "/{}/search/{}".format(nodeType, bNode['name']) if bNode['minTx'] is not None else addrUrl + bNode['addr'],
			"webUrl": addrUrl + bNode['addr'],
			"walletName": bNode['walletName'] or "",
			"value": ((float(bNode['balance'] or 0)/100000000)*btcPrice) if nodeType == 'btc' else float(bNode['balance'] or 0)/100000000,
			"img": img,
			"title": ("<b>{}</b><br> "
						"Address: {}<br> "
						"Balance: {}<br> "
						"Wallet: {}<br> "
						"Last Updated: {} ").format(nodeType.upper(), bNode['addr'], numWithCommas(float(bNode['balance'] or "0") ), 
													"None" if bNode['walletName'] is None or bNode['walletName'] == '' else bNode['walletName'], bNode['lastUpdate'])
		}
		if bNode['label'] != bNode['address']:
			bNode['title'] = ("<b>{}</b><br> "
							"Name: {}<br> "
							"Address: {}<br> "
							"Balance: {}<br> "
							"Wallet: {}<br> "
							"Last Updated: {}").format(nodeType.upper(), bNode['label'], bNode['address'], numWithCommas(bNode['balance']), 
														"None" if bNode['walletName'] is None or bNode['walletName'] == '' else bNode['walletName'], 
														time.strftime("%Y-%m-%d, %H:%M:%S", time.localtime(bNode['lastUpdate'])))
		rel   = nodes.get('r')
		rel   = {
			"from": aNode['id'],
			"to": bNode['id'],
			"id": rel.id,
			"value": float(rel['amount'] or 0)*btcPrice if nodeType == 'btc' else float(rel['amount'] or 0),
			"source": aNode['label'],
			"target": bNode['label'],
			"amount": float(rel['amount'] or 0),
			"txsNum": int(rel['txsNum'] or 1.0),
			"lastUpdate": rel['epoch'] or rel['time'] or time.time(),
			"avgTx": float(rel['avgTxAmt'] or rel['amount'] or 0),
			"avgTxVal": float(rel['avgTxAmt'] or rel['amount'] or 0)*btcPrice if nodeType == 'btc' else float(rel['avgTxAmt'] or rel['amount'] or 0),
			"img": img,
			"color": {
				"color": "#F9A540" if nodeType == 'btc' else "#26A17B"
			},
			"sourceUrl": "/{}/search/{}".format(nodeType, aNode['label']) if aNode['url'] != ''
						else addrUrl + aNode['label'],
			"targetUrl": "/{}/search/{}".format(nodeType, bNode['label']) if aNode['url'] != ''
						else addrUrl + bNode['label'],
			"title": ("Collapsed: True<br> "
						"# of Txs: {}<br> "
						"Total: {}<br> "
						"Average Tx Amount: {}<br> "
						"Last Updated: {}").format(numWithCommas(rel['txsNum'] or 1.0), numWithCommas(rel['amount']), 
										numWithCommas(rel['avgTxAmt'] or rel['amount']), rel['lastUpdate'] or rel['time'])
		}

		aExists = False
		bExists = False
		for node in data['nodes']:
			if node['id'] == aNode['id']:
				aExists = True
			if node['id'] == bNode['id']:
				bExists = True
			if aExists and bExists:
				break
		if not aExists:
			if mainAddr == None and aNode['label'] == id:
				mainAddr = aNode
				mainAddr['minTx'] = nodes.get('a')['minTx']
				mainAddr['tx_since'] = nodes.get('a')['tx_since']
				mainAddr['url'] = addrUrl + mainAddr['address']
			data['nodes'].append(aNode)
		if not bExists:
			if mainAddr == None and bNode['label'] == id:
				mainAddr = bNode
				mainAddr['minTx'] = nodes.get('b')['minTx']
				mainAddr['tx_since'] = nodes.get('b')['tx_since']
				mainAddr['url'] = addrUrl + mainAddr['address']
			data['nodes'].append(bNode)
		data['edges']['collapsed'].append(rel)
	
	for nodes in txs:
		addrUrl = 'https://blockexplorer.com/address/'
		nodeType = 'btc'
		txUrl = 'https://blockexplorer.com/tx/'
		img = "data:image/png;base64,"

		aNode = nodes.get('a')
		if next(iter(aNode.labels)).lower() == 'usdt':
			addrUrl = 'https://omniexplorer.info/address/'
			nodeType = 'usdt'
			txUrl = 'https://omniexplorer.info/tx/'
			img = "https://s2.coinmarketcap.com/static/img/coins/200x200/825.png"

		aNode = {
			"id": aNode.id,
			"label": aNode['name'],
			"isKnown": True if aNode['minTx'] is not None else False
		}
		bNode = nodes.get('b')
		bNode = {
			"id": bNode.id,
			"label": bNode['name'],
			"isKnown": True if bNode['minTx'] is not None else False
		}
		rel   = nodes.get('r')
		rel   = {
			"from": aNode['id'],
			"to": bNode['id'],
			"id": rel.id,
			"value": float(rel['amount'] or 0)*btcPrice if nodeType == 'btc' else float(rel['amount'] or 0),
			"source": aNode['label'],
			"target": bNode['label'],
			"amount": float(rel['amount'] or "0"),
			"time": rel['epoch'],
			"txid": rel['txid'],
			"img": img,
			"color": {
				"color": "#F9A540" if nodeType == 'btc' else "#26A17B"
			},
			"txidUrl": txUrl + rel['txid'],
			"sourceUrl": "/{}/search/{}".format(nodeType, aNode['label']) if aNode['isKnown'] 
						else addrUrl + aNode['label'],
			"targetUrl": "/{}/search/{}".format(nodeType, bNode['label']) if bNode['isKnown'] 
						else addrUrl + bNode['label'],
			"title": ("Collapsed: False<br> "
						"Txid: {}<br> "
						"Total: {}<br> "
						"Time: {}").format(rel['txid'], numWithCommas(float(rel['amount'])), rel['time'])
		}
		data['edges']['all'].append(rel)
	if len(data['edges']['collapsed']) < 1:
		data['edges'] = None
	return render(request, 'coin/coin.html', {'search': mainAddr, 'nodes': data['nodes'], 'edges': data['edges']})